...
function toRGBA(color: string): number[] {
  ...
  if (color.slice(0, 3).toLowerCase() === "rgb") {
    ...
    rgba[3] = d[3] ? (d[3].indexOf("%") !== -1 ? parseInt(d[3]) / 100 : parseFloat(d[3])) : 0;
  } else {
    let d: number;
    if (l < 6)
      d = parseInt(
    ...
    rgba[1] = (d >> 8) & 255;
    rgba[2] = d & 255;
    rgba[3] = l === 9 || l === 5 ? r((((d >> 24) & 255) / 255) * 10000) / 10000 : 0;
  }
  return rgba;
}
 ...
 * @returns {*}
 */
function blend(color1: string, color2: string, p = 0.5, format: "hex" | "rgb" = "hex"): string {
  color1 = color1.trim();
  color2 = color2.trim();
...
console.log(blend(" #46b5d555 ", " rgb(245  245  245  /  80%)", 0.5, "rgb"));
